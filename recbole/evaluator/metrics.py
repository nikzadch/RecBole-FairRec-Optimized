...
class ValueUnfairness(AbstractMetric):
    ...
    def used_info(self, dataobject):
        ...
        if self.mode != 'full':
            ...
            return pos_score, pos_iids, sst_value_dict
    ...
    def calculate_metric(self, dataobject):
        if self.mode != 'full':
            ...
            pos_score, pos_iids, sst_value_dict = self.used_info(dataobject)
            metric_dict = {}
            for sst_key, sst_value in sst_value_dict.items():
                key = 'Value Unfairness of sensitive attribute {}'.format(sst_key)
...
#        pos_score, pos_iids, neg_score, neg_iids, sst_value = self.used_info(dataobject)
        ...
        # return metric_dict
            ...
            metric_dict = {}
            for sst_key, sst_value in sst_value_dict.items():
                key = 'Absolute Unfairness of sensitive attribute {}'.format(sst_key)
...
#            metric_dict.update({f"Relative Difference of NDCG of sensitive attribute {sst} @{(i+1)*5}": round(item, 4)})
                ...
                metric_dict[key] = round(item, self.decimal_place)
...
class NDCG_sub(TopkMetricForsst):
    ...
    respect to their other sensitive attribute which is their age, binray in this case as well, so
    it returns 4 outputs for each topK. (Male, Elder), (Male, Younger), (Female, elder), (Female, Yougner).
